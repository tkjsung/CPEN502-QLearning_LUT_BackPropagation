...
public class MyRobot_20201111_Terminal extends AdvancedRobot{
    ...
    /** Q-Update Required variables  */
    private ArrayList<Integer> qStates = new ArrayList<Integer>();
    private ArrayList<Double> qStateValues = new ArrayList<Double>();
    private ArrayList<Double> qExploreValue = new ArrayList<Double>();
    private ArrayList<Double> qRewards = new ArrayList<Double>();
    ...
    public void run() {
         ...
         * Best way: Toggle a breakpoint after the following line. Enter Debug mode.
         * Add only this tank into Robocode to save time.
         */
//        createLUT(); initializeLUT(); writeLUT(LUT,"LUT_new.txt");
//        initializeRecordFile("winCounter.txt"); initializeRecordFile("rewardTracker.txt");
        ...
        this.qStates.add(this.currentState);
        this.qStateValues.add(this.currentStateValue);
        this.qRewards.add(0.0);
        this.qExploreValue.add(0.0);
        ...
        while (true) {
            ...
            if(exploreChance > this.explore){
                ...
                this.qStates.add(maxState);
                this.qStateValues.add(maxStateValue);

                //TODO: For terminal rewards robot, this gets replaced.

                System.out.println("Adding rewards.");
                this.qRewards.add(this.rewards);
...
//                }
            ...
            // Exploration.
                ...
                int exploreState = this.currentState; // Original path for non-explore
                double exploreStateValue = this.currentStateValue; // "max" value for off-policy
                this.qExploreValue.add(exploreStateValue);
                ...
                tempStringArray[tempStringArray.length-1]= String.valueOf(actionDigit);
                ...
                this.qStates.remove(this.qStates.size()-1);
                this.qStateValues.remove(this.qStateValues.size()-1);
                this.qStates.add(this.currentState);
                this.qStateValues.add(this.currentStateValue);
...
//            writeLUT(LUT,"LUT.txt");
            ...
            System.out.println("Last line in run()");
        } // end while()
    ...
    public void updateStates() {
        Integer[] qStatesArray = new Integer[this.qStates.size()];
        Double[] qStateValuesArray = new Double[this.qStateValues.size()];
        Double[] qExploreValueArray = new Double[this.qExploreValue.size()];
        Double[] qRewardsArray = new Double[this.qRewards.size()];
        System.out.println("Updating States.");
        System.out.println("Sizes of Q-States: " + qStates.size() + qStateValues.size() + qExploreValue.size()
                + qRewards.size());
        double tempStateValue = 0;

        for (int i = 0; i < qStatesArray.length; i++) {
            qStatesArray[i] = this.qStates.get(i);
            qStateValuesArray[i] = this.qStateValues.get(i);
            qExploreValueArray[i] = this.qExploreValue.get(i);
            qRewardsArray[i] = this.qRewards.get(i);
        }

        for (int i = 0; i < qStatesArray.length - 1; i++) {
            if(offPolicy){
                if (qExploreValueArray[i+1] != 0) {
                    System.out.println("Off policy Q-value update.");
                    tempStateValue = qStateValuesArray[i] + this.stepSize * (qRewardsArray[i + 1] +
                            this.gamma * qExploreValueArray[i + 1] - qStateValuesArray[i]);
                    System.out.println(qExploreValueArray[i + 1] + " vs. " + qStateValuesArray[i]);
//                tempStateValue = this.currentStateValue + this.stepSize*(this.rewards +
//                        this.gamma*exploreStateValue-this.currentStateValue);
                }
            }
                ...
                System.out.println("On policy Q-value update.");
                tempStateValue = qStateValuesArray[i] + this.stepSize * (qRewardsArray[i+1] +
                        this.gamma * qStateValuesArray[i+1] - qStateValuesArray[i]);
//                tempStateValue = this.currentStateValue + this.stepSize*(this.rewards +
//                        this.gamma*this.nextStateValue-this.currentStateValue);
            }
            qStateValuesArray[i] = tempStateValue;
        }

        // Update LUT
        for (int j = 0; j < qStatesArray.length; j++) {
            for (int i = 0; i < this.LUT.length; i++) {
                if (this.LUT[i][0].equals(String.valueOf(qStatesArray[j]))) {
                    this.LUT[i][1] = String.valueOf(qStateValuesArray[j]);
                }
            }
        }
    }
    ...
    public void quantize(){
        ...
        this.quantizedEnergy = quantizeEnergy(this.robotEnergy);
